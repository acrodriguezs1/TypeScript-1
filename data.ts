...
export const seriesinfo = [
  ...
  let datosserietable:HTMLElement = document.getElementById("Serie")!;
  let promedio:HTMLElement = document.getElementById("promedio")!;
  
  function mostrarDatosserie(Serie:serie[]):void{

    Serie.forEach(serie => {
        const elemento:HTMLElement = document.createElement('tr');
        elemento.innerHTML = ` <td><b>${serie.numero}</b></td>
            <td>${serie.nombre}</td>
            <td>${serie.channel}</td>
            <td>${serie.seasons}</td>`;

            datosserietable.appendChild(elemento);
    });
}

function suma(Serie:serie[]):number{
  let prom :number=0
  Serie.forEach(serie => {
      prom= prom+serie.seasons
  });
  return prom/Serie.length;
}
promedio.innerHTML=`${suma(seriesinfo)}`;
mostrarDatosserie(seriesinfo);
